# Chart 2: Ek(lambda)/M(mu=1)/1 -> rho=lambda
...
inar1.arl <- function(L, U, lambda, beta, k) {
  ...
  d    <- U-L+1
  qij_ <- function(i, j) {
    if(j==0) 1-pnbinom(i, k, lambda/(1/k+lambda)) else {if (j<=i+1) dnbinom(i-j+1, k, lambda/(1/k+lambda)) else 0}
  }
  qij  <- Vectorize(qij_)
  Q    <- outer(i, i, qij)
  one  <- rep(1, d)
...
inar1.arl2 <- function(L, U, gL, gU, lambda, beta, k) {
  ...
  I     <- diag(1, d)
  cARL  <- solve(I-Q, one)
  #DP    <- dnbinom(i, k, lambda/(1/k+lambda))
  #DP[1] <- (1-gL) * DP[1]
  #DP[d] <- (1-gU) * DP[d]
  #ARL   <- 1 + sum( DP * cARL )
  #ARL
...
# Define UI ----
ui <- fluidPage(
  titlePanel("ARL-unbiased X^n chart - Ek/M/1 queue"),
  sidebarLayout(
    sidebarPanel(
      sliderInput("lambda",
...
server <- function(input, output) {
  ...
  output$ARLProfilePlot <- renderPlot({
    ...
    # Plot
    plot(lambda, LL3, type = 'l', col = 'black', xlab = "ρ", ylab = expression("ARL : E"[5] * "/M/1 " ~ hat(X)[n]))
    abline(v=lambda0, h=ARL0, lty=4, col="grey")
    legend(legend_pos, legend = c(paste("LCL =", 0), paste("UCL =", round(rv$U3, 6)), 
  ...
  output$downloadPlot <- downloadHandler(
    ...
    content = function(file) {
      ...
      plot(lambda, LL3, type = 'l', col = 'black', xlab = "ρ",  ylab = expression("ARL : E"[5] * "/M/1 " ~ hat(X)[n]))
      abline(v=lambda0, h=ARL0, lty=4, col="grey")
      legend(legend_pos, legend = c(paste("LCL =", 0), paste("UCL =", round(U3, 6)), 
