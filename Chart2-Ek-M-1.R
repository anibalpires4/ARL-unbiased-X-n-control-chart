...
inar1.arl <- function(L, U, rho, beta, k) {
  ...
  d    <- U-L+1
  qij_ <- function(i, j) {
    if(j==0) 1-pnbinom(i, k, rho/(1/k+rho)) else {if (j<=i+1) dnbinom(i-j+1, k, rho/(1/k+rho)) else 0}
  }
  qij  <- Vectorize(qij_)
  Q    <- outer(i, i, qij)
  one  <- rep(1, d)
...
inar1.get.UL <- function(ARL0, rho, beta, k, OUTPUT=FALSE) {
  ...
  # We skip the logic to adjust L and just find U
  if (is.na(U)) {
    stop("Unable to find U")
  }
...
# Define UI ----
ui <- fluidPage(
  titlePanel("ARL-unbiased X^n chart"),
  sidebarLayout(
    sidebarPanel(
      sliderInput("rho",
...
server <- function(input, output) {
  ...
  output$ARLProfilePlot <- renderPlot({
    ...
    # Plot
    plot(rho, LL3, type = 'l', col = 'black', xlab = "ρ", ylab = expression("ARL : E"[5] * "/M/1 " ~ hat(X)[n]))
